...
/**
 * Every thread on one file, split by whether it is still open.
 *
 * The split is the page's whole layout: open threads are the work, and
 * resolved ones are folded behind a disclosure rather than dropped. Dropping
 * them would make a resolved thread on a file the column never received
...
/**
 * Every thread, gathered under the file it is on.
 *
 * Built straight on `orderedThreads`, so the page reads top to bottom in the
 * same order the column does and in the same order `n` steps through. That
 * shared ordering is the point: three surfaces disagreeing about where a
 * thread comes in the review is how a reviewer loses one.
 *
 * The grouping relies on that sort putting a file's threads next to each
 * other, which it does — rank, then path, then line.
 */
export function threadGroups(
  threads: readonly ReviewThread[],
  paths: readonly string[],
): FileThreadGroup[] {
  const groups: FileThreadGroup[] = [];
  let group: FileThreadGroup | undefined;

  for (const { thread, inDiff } of orderedThreads(threads, paths)) {
    if (group === undefined || group.path !== thread.path) {
      group = { path: thread.path, inDiff, open: [], resolved: [] };
      groups.push(group);
    }
